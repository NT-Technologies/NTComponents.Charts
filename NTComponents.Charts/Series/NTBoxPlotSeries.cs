...
public class NTBoxPlotSeries<TData> : NTCartesianSeries<TData> where TData : class
{
   ...
   public override void Render(SKCanvas canvas, SKRect renderArea)
   {
      ...
      var visibilityFactor = VisibilityFactor;
         ...
         for (int i = 0; i < dataList.Count; i++)
         {
            ...
            float minPos = Chart.ScaleY(animMin * vFactor, YAxis, renderArea);
            float q1Pos = Chart.ScaleY(animQ1 * vFactor, YAxis, renderArea);
            float medianPos = Chart.ScaleY(animMedian * vFactor, YAxis, renderArea);
            float q3Pos = Chart.ScaleY(animQ3 * vFactor, YAxis, renderArea);
            float maxPos = Chart.ScaleY(animMax * vFactor, YAxis, renderArea);
         ...
         if (boxValues.Outliers != null)
         {
            ...
            {
               float oy = Chart.ScaleY(outlier * vFactor, YAxis, renderArea);
   ...
   public override (int Index, TData? Data)? HitTest(SKPoint point, SKRect renderArea)
   {
      ...
      for (int i = 0; i < dataList.Count; i++)
      {
         ...
         float minPos = Chart.ScaleY(boxValues.Min, YAxis, renderArea);
         float maxPos = Chart.ScaleY(boxValues.Max, YAxis, renderArea);
