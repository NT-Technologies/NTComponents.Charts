       ...
       SKColor? strokeColor = null) where TData : class
   {
      ...
      switch (shape)
      {
         case PointShape.Circle:
            context.Canvas.DrawCircle(x, y, halfSize, paint);
            break;
         case PointShape.Square:
            context.Canvas.DrawRect(x - halfSize, y - halfSize, scaledSize, scaledSize, paint);
            break;
      ...
      if (style == PointStyle.Outlined && strokeColor.HasValue)
      {
         ...
         // The previous draw already did it if style was Stroke, 
         // but if we want an explicit stroke on top of filled? 
         // The original code was a bit ambiguous here.
      }
   }
